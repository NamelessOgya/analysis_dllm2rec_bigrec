...
for p in path:
    ...
    ci_rank = torch.load(f'YOUR_SASRec_MODEL_DIR/movie/SASRec/{key}/val.pt')[:, :-1] # get the SASRec predict
    ...
    from tqdm import tqdm
    g_list= [i / 100 for i in range(100)] + [i for i in range(1, 100)]
    ...
    ci_rank = torch.load(f'YOUR_SASRec_MODEL_DIR/movie/SASRec/{key}/test.pt')[:, :-1]
    assert ci_rank.shape == dist.shape
    ci_min = torch.min(ci_rank, dim=1, keepdim=True)[0]
    ci_max = torch.max(ci_rank, dim=1, keepdim=True)[0]
    ci_norm = (ci_rank - ci_min) / (ci_max - ci_min)
