...
for p in path:
    ...
    if args.popularity_file and args.popularity_gamma > 0:
        ...
        try:
            ...
            # item_dict maps name -> id
            # We need a tensor where index is item_id and value is count
            # Assuming item_ids are contiguous 0..N-1
            ...
            # Normalize pop_rank as in adjust_pda.py
            ...
            # dist is [batch/num_test, num_items]
            # pop_rank should be [1, num_items] or broadcastable
            pop_rank = pop_rank.to(dist.device).unsqueeze(0) # [1, num_items]
            
            # Apply formula: rank = (1 + pop)^(-gamma) * dist
            # Note: In adjust_pda.py: rank = torch.pow((1 + pop_rank), -gamma) * dist
    ...
    for topk in topk_list:
        ...
        for i in range(len(test_data)):
            ...
            if target_id is None:
                continue
            minID = rank_indices[i][target_id].item()
            if minID < topk:
...
f = open('./debug_single_game.json', 'w')    
